...
class Manager extends Employee {
    ...
    calculateBonus() {
        return this.salary * 12
    };// Calculates Manager annual salary
};
const mgr1 = new Manager("John Smith", 201, "IT", 8000, 5);// Adds new manager to Manager class
...
class Company {
    ...
    addEmployee(employee) {
        this.employees.push(employee);// Adds the Employee class to Company class
    };
    listEmployees() {
    ...
    calculateTotalPayroll() {
        return this.employees.reduce((salary, employee) => {
            return salary + employee.calculateAnnualSalary();
        }, 0);// Calculates combined salary of both Employee and Manager
    };
// Task 5 - Implementing Promotions
    promoteToManager(employee, teamSize) {
        if (employee instanceof Manager) {
            ...
            return;
        }// Checks if person is already a manager
        const manager = new Manager(
            ...
            employee.salary,
            teamSize
        );// Creates a new manager object with same details
        const index = this.employees.findIndex(emp => emp.id === employee.id);
        if (index !== -1) {
            this.employees[index] = manager;
        };// Searches in the employee index and promotes them to manager
    };
};
